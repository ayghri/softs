#!/usr/bin/env python3
"""Distillation broker - run in its own process/terminal.

    python broker.py

Reads ``conf/distill.yaml`` (Hydra), so any field can be overridden on the CLI,
e.g. ``python broker.py log_level=DEBUG``.
"""
