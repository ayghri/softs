#!/usr/bin/env python3
"""Single-layer distillation teacher supplier - run in its own process (one GPU).

    python supplier.py device.worker_gpu=0

Captures a decoder layer's (input, output) hidden states with ModelIOCatcher
(early-exit), serving any layer via one regex pattern.
"""
...
def main(cfg: DictConfig):
    ...
    supplier = Supplier(
        ...
        product_ids=[SUPPLIER_PATTERN],  # regex: serves any layer
